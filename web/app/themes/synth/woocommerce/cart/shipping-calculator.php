...
<form class="shipping_calculator" action="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" method="post">
	...
	<section class="shipping-calculator" style="display: block;">
		...
		<p class="form-row form-row-wide">
			<select name="calc_shipping_country" id="calc_shipping_country" class="country_to_state span4 form-control col-md-12" rel="calc_shipping_state">
		...
		<p class="form-row form-row-wide">
			<?php
				...
				// Dropdown Input
					...
					?><span>
						<select name="calc_shipping_state" id="calc_shipping_state" placeholder="<?php _e( 'State / county', 'woocommerce' ); ?>" class="col-md-12 shop-select shop-confirm">
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_city', false ) ) : ?>
			...
			<p class="form-row form-row-wide">
				<input type="text" class="form-control" value="<?php echo esc_attr( $woocommerce->customer->get_shipping_city() ); ?>" placeholder="<?php _e( 'City', 'woocommerce' ); ?>" name="calc_shipping_city" id="calc_shipping_city" />
			</p>
		...
		<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) : ?>
			...
			<p class="form-row form-row-wide">
				<input type="text" class="form-control" value="<?php echo esc_attr( $woocommerce->customer->get_shipping_postcode() ); ?>" placeholder="<?php _e( 'Postcode / Zip', 'woocommerce' ); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
			</p>

		<?php endif; ?>

		<button type="submit" name="calc_shipping" value="1" class="btn btn-primary"><?php _e( 'Update Totals', 'woocommerce' ); ?></button>
